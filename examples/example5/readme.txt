...
get raw return of portfolios for all stocks, and the
portfolios are constructed through a equal weighted way.
...
<way_weight> 'ew' </way_weight> # construct equal weighted portfolios.
<Is_construct_model> 'no' </Is_construct_model> # not construct factor model
<Is_portfolio_regression> 'no' </Is_portfolio_regression> # not calculate the regression of portfolio raw return on factor model
...
or users can just replace file './codes/parameter.xml' by the file  './examples/example5/parameter.xml'.
