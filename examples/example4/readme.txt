For example1, users want to calculate value of predictors and
get raw return of portfoliosfor all stocks, then users try to calculate the risk-adjusted
return by using the CH3 model (chinese 3 factor model by liu, et al (2019) from paper 'Size and value in China'),
then uses try to obtain the rolling regression results based on CH3 factor model with
the rolling periods 10 months.
...
<reg_model> 'ch3' </reg_model> # factor model is CH3
<Is_portfolio_regression_rolling> 'yes' </Is_portfolio_regression_rolling> # not calculate the rolling regression of portfolio raw return on factor model
<rolling_period type="number"> 10 </rolling_period> # rolling periods 10 months

Users can modify the file './codes/parameter.xml' according the above description,
or users can just replace file './codes/parameter.xml' by the file  './examples/example4/parameter.xml'.
